// Package table facilitates rendering column-formatted data to a terminal and TSV-formatted data to
...
import (
	...
	"github.com/sbchaos/opms/lib/text"
)
...
func New(w io.Writer, isTTY bool, maxWidth int) Printer {
	if isTTY {
		return &ttyPrinter{
			out:      w,
			maxWidth: maxWidth,
...
func (t *ttyPrinter) AddHeader(columns []string, opts ...fieldOption) {
	if t.hasHeaders {
		return
	}
...
func (t *ttyPrinter) AddField(s string, opts ...fieldOption) {
	if t.rows == nil {
...
func (t *ttyPrinter) calculateColumnWidths(delimSize int) []int {
	numCols := len(t.rows[0])
